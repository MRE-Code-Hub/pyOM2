...
!=======================================================================
 ...
 !---------------------------------------------------------------------------------
  ...
  ! note: this was a bug corrected March 19 2020
  if (enable_idemix .and. .not. enable_idemix3) then
     RiNumber= min(RiNumber,KappaM*Nsqr(:,:,:,tau)/max(1d-12,alpha_c*E_iw(:,:,:,tau)**2))
  endif   
  if (enable_idemix .and. enable_idemix3) then
     RiNumber= min(RiNumber,KappaM*Nsqr(:,:,:,tau)/max(1d-12,iw_diss(:,:,:)))
  endif   
...
!=======================================================================
 ...
 !---------------------------------------------------------------------------------
 ! Sources and sinks by vertical friction and vertical mixing
 !---------------------------------------------------------------------------------
 forc = K_diss_v - P_diss_v 

 !---------------------------------------------------------------------------------
 ! sink by implicit numerical mixing
 !---------------------------------------------------------------------------------
 if (enable_take_p_diss_adv_from_tke) forc = forc - P_diss_adv
 ...
 !---------------------------------------------------------------------------------
  ...
  if (enable_eke) then
   forc = forc + eke_diss_iw 
      ...
      forc = forc + K_diss_gm - P_diss_skew
 ...
 !---------------------------------------------------------------------------------
  ...
  do k=1,nz
   do j=js_pe,je_pe
    do i=is_pe-1,ie_pe
       flux_east(i,j,k)=K_h_tke*(tke(i+1,j,k,tau)-tke(i,j,k,tau))/(cost(j)*dxu(i))*maskUtr(i,j,k)
    enddo
   enddo
   do j=js_pe-1,je_pe
    do i=is_pe,ie_pe
       flux_north(i,j,k)=K_h_tke*(tke(i,j+1,k,tau)-tke(i,j,k,tau))/dyu(j)*maskVtr(i,j,k)*cosu(j)
    enddo
   enddo
  enddo
 ...
 if (enable_tke_dst3_advection) then
  call adv_flux_dst3_wgrid(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,nz,flux_east,flux_north,flux_top,tke(:,:,:,tau))
 endif
 if (enable_tke_superbee_advection .or. enable_tke_dst3_advection) then
  do j=js_pe,je_pe
   do i=is_pe,ie_pe
  ...
  if (enable_tke_AB_time_stepping ) then 
    !---------------------------------------------------------------------------------
    ! Adam Bashforth time stepping
    !---------------------------------------------------------------------------------
     tke(:,:,:,taup1)=tke(:,:,:,taup1)+dt_tracer*( (1.5+AB_eps)*dtke(:,:,:,tau) &
                         - ( 0.5+AB_eps)*dtke(:,:,:,taum1))
  else
     tke(:,:,:,taup1)=tke(:,:,:,taup1)+dt_tracer*dtke(:,:,:,tau) 
  endif
  !---------------------------------------------------------------------------------
  ! open boundaries
  !---------------------------------------------------------------------------------
  call set_obc_tracer_wgrid(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,nz,tke,.true.) 
 endif
