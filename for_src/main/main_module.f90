...
!---------------------------------------------------------------------------------
      ...
      logical :: enable_thermodynamic_equation     = .true.  ! calculate changes in temperature and salinity
      ...
      logical :: enable_tempsalt_sources           = .false. ! enable restoring zones for temp/salt
      logical :: enable_momentum_sources           = .false. ! enable restoring zones for u/v with Euler forward time stepping
      logical :: enable_momentum_sources_with_AB   = .false. ! enable restoring zones for u/v with AB time stepping
      logical :: enable_superbee_advection         = .false. ! enable advection scheme with implicit mixing
      logical :: enable_upwind3_advection          = .false. ! enable advection scheme with implicit mixing
      logical :: enable_dst3_advection             = .false. ! enable advection scheme with implicit mixing
      logical :: enable_AB_time_stepping           = .true.  ! Adam Bashforth or foreward time stepping for advection
      ...
      logical :: enable_store_cabbeling_heat       = .false. ! transfer non-linear heating terms to potential enthalpy
                                                             ! else transfer to TKE and EKE
      logical :: enable_take_P_diss_adv_from_tke   = .false. ! take energy for implicit mixing by advection from tke
...
!---------------------------------------------------------------------------------
      ...
      real*8, allocatable, dimension(:,:)     :: area_z     ! Area of Z-box in m^2
...
!=======================================================================
 ...
 implicit none
  ...
  allocate( area_z(is_pe-onx:ie_pe+onx,js_pe-onx:je_pe+onx) ); area_z = 0
  ...
  if (enable_momentum_sources .or. enable_momentum_sources_with_AB) then
